...
def get_asset_from_ipfs(cid):
    response = requests.get(str("https://api.nft.storage/bafybeihtn6sk44pxizjedv7u2nqdq3ncijwmcdxgin3tcnnqh55hckvpfm"), headers=get_headers("application/json"))
    print(response.json())
...
def upload_asset_to_ipfs(asset_file, format):
    ...
    try:
        ...
        response = requests.post(url=IPFS_URL_PREFIX + "upload/", data=open(asset_file, 'rb'), headers=get_headers(format))
        print(response.json())
        if response.ok == True:
