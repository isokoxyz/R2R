...
class KadcarsUpgrades():
    def attach_nft(dest_object):
        permanent_node_names = ["Principled BSDF",
                                "Material Output", "BAKED_TEXTURE"]
        ...
        configure_bake_settings('CYCLES', 'CUDA', 'GPU',
                                False, False, False, 'DIFFUSE')
        ...
        bpy.ops.object.bake('INVOKE_DEFAULT', type='DIFFUSE', pass_filter={
                            'COLOR'}, save_mode='EXTERNAL', target='IMAGE_TEXTURES')
        ...
        node_tree.links.new(
            bake_texture_node.outputs['Color'], bsdf.inputs['Base Color'])
